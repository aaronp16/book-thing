	...
	>
		...
		>
			...
			<div class="flex min-h-0 flex-1 divide-x divide-neutral-800">
							...
							<div class="grid grid-cols-3 gap-3 overflow-y-auto pr-1">
								...
								>
									...
									{:else}
										...
										>
											...
											<span class="text-center text-xs leading-tight">Upload<br />cover</span>
										</div>
									{/if}
								</button>
								...
								{#each covers as cover (cover.url)}
									...
									>
										...
										{/if}
										<!-- Source label: always visible for embedded, hover for others -->
										<div
											class="absolute right-0 bottom-0 left-0 bg-black/60 px-1.5 py-1 text-center transition-opacity {cover.source ===
											'embedded'
												? 'opacity-100'
												: 'opacity-0 group-hover:opacity-100'}"
										>
											<span
												class="text-xs {cover.source === 'embedded'
													? 'font-medium text-green-400'
													: 'text-neutral-300'}"
											>
												{cover.source === 'embedded' ? 'From book' : cover.source}
											</span>
										</div>
									</button>
								{/each}

								<!-- Empty state when no search results (upload tile is still shown above) -->
								{#if covers.length === 0 && !coversLoading}
									<div class="col-span-2 flex items-center">
										<p class="text-sm text-neutral-500">
											{coversError ?? 'No covers found.'} Upload one using the tile on the left.
										</p>
									</div>
								{/if}
							</div>
				...
				<div class="flex {isEdit ? 'w-56 shrink-0' : 'flex-1'} flex-col overflow-hidden p-4">
					<p class="mb-3 shrink-0 text-sm font-medium text-neutral-300">
