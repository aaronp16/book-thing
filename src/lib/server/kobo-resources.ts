...
/**
 * Hardcoded fallback resources matching the Kobo store API.
 * Used when the proxy fetch to storeapi.kobo.com fails.
 * Sourced from calibre-web's NATIVE_KOBO_RESOURCES.
 */
function getNativeKoboResources(): Record<string, unknown> {
	return {
		...
		facebook_sso_page:
			'https://authorize.kobo.com/signin/provider/Facebook/login?returnUrl=http://kobo.com/',
		...
		more_sign_in_options:
			'https://authorize.kobo.com/signin?returnUrl=http://kobo.com/#allProviders',
		...
		provider_external_sign_in_page:
			'https://authorize.kobo.com/ExternalSignIn/{providerName}?returnUrl=http://kobo.com/',
...
export function createKoboResourcePayload(options: {
	...
	// Start with native fallback, then overlay any store-fetched resources, then our overrides
	const resources: Record<string, unknown> = {
		...getNativeKoboResources(),
		...(options.baseResources ?? {})
	};

	// Override endpoints that we handle locally
	resources.library_sync = `${shelfBase}/v1/library/sync`;
	resources.image_host = baseUrl;
	// Use path-based cover URLs matching Kobo's expected template format
	resources.image_url_template = `${shelfBase}/covers/{ImageId}/{Width}/{Height}/false/image.jpg`;
	resources.image_url_quality_template = `${shelfBase}/covers/{ImageId}/{Width}/{Height}/{Quality}/{IsGreyscale}/image.jpg`;
